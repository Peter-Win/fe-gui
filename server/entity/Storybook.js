...
const {injectStorybookRule} = require('../sysUtils/injectStorybookRule')
...
class Storybook {
    ...
    async init() {
        const {entities: {PackageJson, React}} = require('./all')
        this.isInit = false
        this.isReady = false
        if (!React.isInit) return
        await PackageJson.load()
        this.isInit = PackageJson.isDevDependency('@storybook/react')
        this.isReady = !this.isInit
    }

    defaultParams = {
        removeStdExample: false,
        createCustomExample: true,
    }
    ...
    async create(params) {
        ...
        if (params.removeStdExample) {
            ...
            wsSendCreateEntity(this.name, `Remove folder: ${dirName}`)
        ...
        if (params.createCustomExample) {
            ...
            const exampleFiles = [
                ...
                'RatingBar.jsx',
                'RatingBar.tsx',
                'RatingBar.stories.jsx',
                'RatingBar.stories.tsx',
            ]
            for (const shortName of exampleFiles) {
                const ext = path.extname(shortName).toLowerCase()
                console.log('shortName=', shortName, ', ext = ', ext, ', lang=', CommonInfo.tech.language)
                if (/^\.js/.test(ext) && CommonInfo.tech.language !== 'JavaScript') continue
                if (/^\.ts/.test(ext) && CommonInfo.tech.language !== 'TypeScript') continue
                // RatingBar uses CSS
                if (shortName.startsWith('RatingBar') && !CommonInfo.tech.styleCss) continue
                wsSendCreateEntity(this.name, `* ${shortName}`)
                await buildTemplate(shortName, path.join(dirName, shortName), {})
            }
        }
    }
...
<div>
  ...
  <a href="https://storybook.js.org/" target="_blank">Official site</a>
</div>    
`
    controls = `
<div>
  <div class="rn-ctrl" data-name="removeStdExample" data-type="Checkbox" data-title="Remove standard example in 'storybook' folder"></div>
  <div class="rn-ctrl" data-name="createCustomExample" data-type="Checkbox" data-title="Create custom example"></div>
</div>
    `
